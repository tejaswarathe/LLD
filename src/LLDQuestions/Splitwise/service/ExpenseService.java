...
import LLDQuestions.Splitwise.models.User;
...
public class ExpenseService {
    ...
    private void divideExpenses(Float amount, String payingUser, Map<String, Float> splitMap, SplitType splitType) {
    ...
    private void divideExpenseEqually(Float amount, String payingUser) {
        ...
        for (String userId : splitdata.userMap.keySet()) {
            User currentUser = splitdata.getUserMap().get(userId);
            Float share = getSplitShare(amount / splitdata.userMap.keySet().size(), amount, SplitType.EQUAL, isFirstUser);
            isFirstUser = false;
            if (!userId.equals(payingUser)) {
                // update data in current user
                Float balanceOfPayingUserInCurrentUser = currentUser.getBalances().get(payingUser);
    ...
    public void getExpense(String userId) {
        ...
        for (String fellowUserId : currentUser.getBalances().keySet()) {
            ...
            if (fellowUserBalance < 0) {
                System.out.println(currentUser.getName() + " owes " + fellowUser.getName() + ": " + Math.abs(fellowUserBalance));
            } else if (fellowUserBalance > 0) {
                System.out.println(fellowUser.getName() + " owes " + currentUser.getName() + ": " + Math.abs(fellowUserBalance));
            }
        }
